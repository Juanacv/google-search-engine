define(["ajaxcall"], function (ajaxcall) { 
  ...
  function getPage(startIndex, active, firstPage) {
    	...
    	ajaxcall.getData({callback:function(response) { 
    		if (response !== "error") {
    			layout.printIt(response, firstPage, active); 
    		}
  ...
  function addEvents(totalResults) {
	...
	var pagination_anchors = document.querySelectorAll(".page_link");
	for (var i = 0; i < pagination_anchors.length ; i++) {
		 //adding event listener to every anchor
    	pagination_anchors[i].addEventListener("click", 
        	function (event) {
