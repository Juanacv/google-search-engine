define(["events"], function (events) {
    ...
    return {
    	printIt:function(result, firstPage, active) {
			...
			for (var i=0; i < 4; i++) {
				if (typeof obj.items[i].pagemap != "undefined" && typeof  obj.items[i].pagemap.cse_thumbnail != "undefined"
				&& typeof obj.items[i].pagemap.cse_thumbnail[0].src != "undefined") {
					document.getElementById("image-"+i).src = obj.items[i].pagemap.cse_thumbnail[0].src
				}
			}
